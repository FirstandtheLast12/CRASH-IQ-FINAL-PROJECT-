...
func _show_cycle_result(cycle_num: int, _trade_data: Dictionary) -> void:
	...
	var current_value: float = SimulationManager.get_portfolio_value()
	var prev_value: float = SimulationManager.starting_cash
	var history: Array = SimulationManager.cycle_history
	if history.size() >= 2:
		prev_value = float(history[history.size() - 2].get("portfolio_value", SimulationManager.starting_cash))
	var cycle_pnl: float = current_value - prev_value

	var pnl_color: Color = Color("00ff41") if cycle_pnl >= 0.0 else Color("ff4444")
	var pnl_prefix: String = "+" if cycle_pnl >= 0.0 else ""
	...
	style.bg_color = Color(0.04, 0.04, 0.04, 0.97)
	style.border_width_left = 3
	style.border_width_top = 3
	style.border_width_right = 3
	style.border_width_bottom = 3
	style.border_color = pnl_color
	...
	cycle_lbl.text = "CYCLE %d / %d COMPLETE" % [cycle_num, SimulationManager.get_cycle_count()]
	cycle_lbl.add_theme_font_size_override("font_size", 18)
	cycle_lbl.add_theme_color_override("font_color", Color(1.0, 1.0, 1.0, 0.70))
	cycle_lbl.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	cycle_lbl.mouse_filter = Control.MOUSE_FILTER_IGNORE

	var pnl_lbl: Label = Label.new()
	pnl_lbl.text = "%s%s this cycle" % [pnl_prefix, _format_currency(cycle_pnl)]
	pnl_lbl.add_theme_font_size_override("font_size", 32)
	pnl_lbl.add_theme_color_override("font_color", pnl_color)
	...
	total_lbl.text = "Portfolio total: %s" % _format_currency(current_value)
	total_lbl.add_theme_font_size_override("font_size", 14)
	total_lbl.add_theme_color_override("font_color", Color(1.0, 1.0, 1.0, 0.50))
	...
	vbox.add_child(pnl_lbl)
	vbox.add_child(total_lbl)
	margin.add_child(vbox)
	banner.add_child(margin)

	var banner_w: float = 520.0
	var banner_h: float = 130.0
	var viewport_size: Vector2 = get_viewport_rect().size
	var target_x: float = (viewport_size.x - banner_w) / 2.0
	var target_y: float = (viewport_size.y - banner_h) / 2.0

	banner.set_anchors_preset(Control.PRESET_TOP_LEFT)
	banner.custom_minimum_size = Vector2(banner_w, banner_h)
